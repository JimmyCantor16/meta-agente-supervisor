...
export function PublishGuide({ projects }: { projects: ProjectSummary[] }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-ink">🚀 {g.title}</h1>
        <p className="mt-1 max-w-2xl text-sm text-ink-muted">{g.intro}</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <label className="text-sm font-semibold text-ink-body" htmlFor="sel-proyecto">
          {g.forProject}
        </label>
        <select
          id="sel-proyecto"
          ...
          className="rounded-xl border border-black/10 bg-white px-3 py-2 text-sm font-medium text-ink-body focus:border-brand-300 focus:outline-none"
        >
          ...
          <button
            ...
            className={`rounded-xl px-4 py-2.5 text-sm font-bold transition ${
              paso === n
                ...
                : "border border-black/10 bg-white text-ink-body hover:border-brand-300"
            }`}
          >
            {icon} {label}
          </button>
      ...
      {paso === 2 && (
        <Card title={`🐙 ${g.step2Title}`} icon={<span>2️⃣</span>}>
          <Leccion concepto={g.step2Why}>
            ...
            <p className="ml-6 text-sm text-ink-muted">{g.step2bNote(proyecto)}</p>
      ...
      {paso === 3 && (
        <Card title={`🌍 ${g.step3Title}`} icon={<span>3️⃣</span>}>
          <Leccion concepto={g.step3Why}>
            ...
            <ul className="ml-6 list-disc space-y-1 text-sm text-ink-body">
              <li><b>Build command:</b> <code className="rounded bg-surface-muted px-1.5 py-0.5 font-mono text-xs">cd backend && npm install</code></li>
              <li><b>Start command:</b> <code className="rounded bg-surface-muted px-1.5 py-0.5 font-mono text-xs">cd backend && npm start</code></li>
              <li>{g.step3cFree}</li>
            </ul>
            ...
            <p className="mt-2 text-xs text-ink-faint">{g.step3Note}</p>
            ...
            <div className="mt-4 rounded-2xl border-2 border-brand-200 bg-brand-50/60 p-4">
              ...
              <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                <input
                  ...
                  className="flex-1 rounded-xl border border-black/10 bg-white px-4 py-2.5 text-sm text-ink placeholder:text-ink-faint focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-100"
                />
                <button
      ...
      <button
        ...
        className={`rounded-xl px-4 py-2 text-sm font-bold transition ${
          hecho
            ...
            : "border border-black/10 bg-white text-ink-body hover:border-emerald-400"
        }`}
      >
...
function Punto({
  ...
  return (
    <p className="flex gap-2 text-sm text-ink-body">
    ...
    <div className="ml-6">
      <div className="relative">
        <button
          ...
          className="absolute right-2 top-2 rounded-lg border border-slate-600 bg-slate-800 px-2.5 py-1 text-xs font-semibold text-ink-faint transition hover:bg-slate-700"
        >
