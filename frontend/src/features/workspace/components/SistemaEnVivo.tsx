  ...
  return (
    <div className="rounded-2xl border border-black/10 bg-white p-4 shadow-sm">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            {corriendo ? (
              ...
              <p className="text-xs text-ink-faint">{g.apagadoHint}</p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {corriendo ? (
            <button
              onClick={() => void apagar()}
              disabled={trabajando}
              className="rounded-xl border border-black/10 px-4 py-2 text-sm font-semibold text-ink-body transition hover:border-black/10 disabled:opacity-50"
            >
              {trabajando ? "…" : g.apagar}
            </button>
          ) : (
      ...
      {trabajando && !corriendo && (
        <p className="mt-2 text-xs text-ink-faint">{g.encendiendoHint}</p>
      )}
  ...
  return (
    <div className="mt-3 border-t border-black/10 pt-3">
      <button
        ...
        className="flex w-full items-center justify-between text-sm font-semibold text-ink-body"
      >
        <span>🔐 {g.titulo}</span>
        ...
        <div className="mt-2 space-y-2 text-xs text-ink-muted">
          ...
          {info ? (
            <>
              <p>
                {g.carpeta}:{" "}
                <code className="break-all rounded bg-surface-muted px-1.5 py-0.5 font-mono text-ink-body">
                  {info.carpeta}
                </code>
              </p>
              <p>
                ...
                {info.nombres.length === 0 ? (
                  <span className="text-ink-faint">{g.ninguna}</span>
                ) : (
                  info.nombres.map((n) => (
                    <span
                      key={n}
