  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p className="text-sm font-bold text-slate-700">🎯 {g.titulo}</p>
        ...
        <div className="mt-3 flex gap-2">
          <input
            ...
            disabled={creando}
            className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-brand-300 focus:bg-white focus:outline-none"
          />
          <button
            onClick={() => void crear()}
      ...
      {cargando && <p className="text-sm text-slate-400">{g.cargando}</p>}
...
function MetaCard({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <p className="text-base font-bold text-slate-800">{meta.objetivo}</p>
      <p className="mt-1 text-sm text-slate-500">{meta.resumen}</p>
...
function HitoRow({ hito, onToggle }: { hito: HitoProceso; onToggle: (hecho: boolean) => void }) {
  ...
  const badge = {
    ...
    tiempo: { txt: g.depTiempo, cls: "bg-slate-200 text-slate-600" },
  }[hito.depende_de];
    ...
    <li className="flex items-start gap-3 rounded-xl border border-slate-100 p-3">
      ...
      <div className="flex-1">
        <div className="flex flex-wrap items-center gap-2">
          <span className={`text-sm font-semibold ${hito.hecho ? "text-slate-400 line-through" : "text-slate-700"}`}>
            {hito.titulo}
          </span>
        ...
        {hito.descripcion && <p className="mt-0.5 text-xs text-slate-500">{hito.descripcion}</p>}
      </div>
    </li>
  );
}
